...
typedef struct
{
	Usr_LightWidget Instance;
} Usr_LightWidget__ESD;


void *Usr_LightWidget__Create__ESD()
{
	Usr_LightWidget__ESD *context = (Usr_LightWidget__ESD *)esd_malloc(sizeof(Usr_LightWidget__ESD));
	if (context)
	{
		Usr_LightWidget__Initializer(&context->Instance);
		context->Instance.Owner = context;
	}
	else
	{
		eve_printf_debug("Out of memory (Usr_LightWidget__Create__ESD)");
	}
	return context;
}

void Usr_LightWidget__Destroy__ESD(void *context)
{
	esd_free(context);
}
